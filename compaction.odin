...
compaction_experiment_and_dump :: proc(shaking_parameter: f64, directory: string, dump_results: bool) {
	...
	for wall in &walls {
	    using wall
	    velocity = E_y * shaker_amplitude * 2 * math.PI * shaker_frequency * math.cos(2 * math.PI * shaker_frequency * time)
	    center_position =
	...
	for i in 0 ..< frame_duration_steps {
		...
		if flip_flop {
		    shaking_func(walls, initial_center_positions, shaker_amplitude, shaker_frequency, dt * current_shaker_step)
		} else {
