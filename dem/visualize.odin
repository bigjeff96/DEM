        ...
        {     //microui
            ...
            if mu.window(ctx, "Sim options", {0, 0, 300, GetScreenHeight() + 100}, opts) {
                ...
                for sphere in spheres {
                    using sphere
                    total_energy +=
                        mass * dot(velocity, velocity) / 2. +
                        moment_of_inertia * dot(angular_velocity, angular_velocity) / 2.
                }
                ...
                mu.label(
                    ctx,
                    tprintf("Total energy: %.6f%%", 100. * total_energy / (max_kinectic_energy * f64(len(spheres)))),
                )
...
debug_sim_code :: proc() {
    ...
    //physics
    total_chains := 1
    total_particles_per_chain := 30
    radius: f64 = 1e-3
    current_time: f64 = 0
    density_glass_beads: f64 = 2500.0
    box_length_x := 3 * 2 * radius
    box_length_z := 31 * 2 * radius
    box_length_y := 31 * 2 * radius
            ...
            if mu.window(ctx, "Sim options", {0, 0, 300, GetScreenHeight() + 100}, opts) {
                ...
                mu.label(
                    ...
                    tprintf(
                        ...
                        100. * total_energy / (max_kinectic_energy * f64(len(spheres))),
                    ),
                )
                mu.label(ctx, tprintf("Density ratio: %.5f%%", 100. * (volume_all_spheres / volume_pile)))
                mu.label(ctx, tprintf("Max heigth: %.10f", max_height))
...
render_cells :: proc(cell_context: ^Cell_context, cell_ids: []i32, walls: []Wall) {
    ...
    for neighbor_id in cell_ids do if neighbor_id != 0 {
            DrawCubeWires(position = vec3_to_rl(cell_coords_unique_id(neighbor_id, cell_context, walls)) * f32(SCALING_FACTOR), width = auto_cast (cell_length * SCALING_FACTOR), height = auto_cast (cell_length * SCALING_FACTOR), length = auto_cast (cell_length * SCALING_FACTOR), color = BLUE)
        }
}
...
render_cell :: proc(cell_context: ^Cell_context, cell_id: int, walls: []Wall, color: rl.Color) {
    ...
    using cell_context
    DrawCube(
        position = vec3_to_rl(cell_coords_unique_id(auto_cast cell_id, cell_context, walls)) * f32(SCALING_FACTOR),
        width = auto_cast (cell_length * SCALING_FACTOR),
        height = auto_cast (cell_length * SCALING_FACTOR),
        length = auto_cast (cell_length * SCALING_FACTOR),
        color = color,
